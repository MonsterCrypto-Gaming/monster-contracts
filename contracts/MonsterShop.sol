...
>>>>>>> 59180407e3e796cddc3a2624a5a752539cb6b3cd
...
contract NftMarketplace is ReentrancyGuard {
    ...
    mapping(address => mapping(uint256 => uint256)) nftContractIndexToId;
    mapping(uint => bool) public IdToListingStatus;
        ...
        isOwner(nftAddress, tokenId, msg.sender)
    {
        ...
        s_listings[nftAddress][tokenId] = Listing(price, msg.sender, tokenId);
        onMarketArray.push(int(tokenId));
        tokenIdToListIndex[tokenId] = int(onMarketArray.length - 1);
        IdToListingStatus[tokenId] = true;
        ...
        isListed(nftAddress, tokenId)
    {
        ...
        //int indexToRemove = tokenIdToListIndex[tokenId];
        remove(uint(tokenIdToListIndex[tokenId]));
        IdToListingStatus[tokenId] = false;
        tokenIdToListIndex[tokenId] = -1;
...
contract NftMarketplace is ReentrancyGuard {
    ...
    mapping(address => uint256) private s_proceeds;
    mapping(address => uint[]) public allNftListings;
        ...
        isOwner(nftAddress, tokenId, msg.sender)
    {
        ...
        allNftListings[nftAddress].push(tokenId);
        emit ItemListed(msg.sender, nftAddress, tokenId, price);
    }
    ...
    {
        delete (s_listings[nftAddress][tokenId]);
        allNftListings[nftAddress].pop(tokenId);
        emit ItemCanceled(msg.sender, nftAddress, tokenId);
    }
